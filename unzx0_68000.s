...
zx0_copy_match:
                ...
                dbf d0,zx0_copy_match   ; loop for all matched bytes
...
zx0_get_offset:
                ...
                bsr.s zx0_elias_loop    ; read high byte of match offset
                ...
                bcs.s zx0_do_copy_offs  ; if len bit is set, no need for more
                bsr.s zx0_elias_bt      ; read rest of elias-encoded match length
                bra.s zx0_do_copy_offs  ; go copy match

zx0_get_elias:
                moveq #1,d0             ; initialize value to 1
zx0_elias_loop:
                add.b d1,d1             ; shift bit queue, high bit into carry
                bne.s zx0_got_bit       ; queue not empty, bits remain
                move.b (a0)+,d1         ; read 8 new bits
                addx.b d1,d1            ; shift bit queue, high bit into carry
                                        ; and shift 1 from carry into bit queue

zx0_got_bit:
                bcs.s zx0_got_elias     ; done if control bit is 1
zx0_elias_bt:
                add.b d1,d1             ; read data bit
                addx.l d0,d0            ; shift data bit into value in d0
                bra.s zx0_elias_loop    ; keep reading

zx0_done:
                movem.l (sp)+,d2/a2     ; restore preserved registers
zx0_got_elias:
                rts
